...
public class RouletteGame {
    ...
    public static double evaluate(RouletteBet bet, RouletteNumber result) {
        return switch (bet.type()) {
            ...
            case GREEN -> result.color().equals("green") ? bet.amount() * 10 : 0;
            case EVEN -> result.number() > 0 && result.number() % 2 == 0 ? bet.amount() * 2 : 0;
            case ODD -> result.number() > 0 && result.number() % 2 != 0 ? bet.amount() * 2 : 0;
            case DOZEN_1 -> result.number() >= 1 && result.number() <= 12 ? bet.amount() * 2 : 0;
            case DOZEN_2 -> result.number() >= 13 && result.number() <= 24 ? bet.amount() * 2 : 0;
            case NUMBER -> result.number() == bet.number() ? bet.amount() * 20 : 0;
        };
    }
}
